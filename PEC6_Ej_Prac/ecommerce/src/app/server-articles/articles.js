...
router.get('/', (req, res) => {
  ...
  if (query) {
    const foundArticles = articles.filter(({ name }) => name.toLowerCase().indexOf(query) !== -1);
...
router.post('/', (req, res) => {
  ...
  if (article.id) {
    return res.status(400).json({ msg: 'Article Id seems to already have an id assigned' });
  }
...
router.patch('/:id', (req, res) => {
  ...
  return res.status(400).json({ msg: 'Article with id ' + articleId + ' not found.' });
});
